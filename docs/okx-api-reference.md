...
### DEV-012 模拟盘只读 REST 核对（2026-09-25）

已核对[模拟盘服务](https://www.okx.com/docs-v5/en/#overview-demo-trading-services)、[REST 鉴权](https://www.okx.com/docs-v5/en/#overview-rest-authentication)、[系统时间](https://www.okx.com/docs-v5/en/#public-data-rest-api-get-system-time)、[账户配置](https://www.okx.com/docs-v5/en/#trading-account-rest-api-get-account-configuration)、[费率](https://www.okx.com/docs-v5/en/#trading-account-rest-api-get-fee-rates)与[限速说明](https://www.okx.com/docs-v5/en/#overview-rate-limit)。模拟盘请求包含 `x-simulated-trading: 1`；私有 GET 的签名原文为 UTC 毫秒 ISO 时间戳 + `GET` + 含查询串路径，使用 HMAC-SHA256 后 Base64。系统时间文档为 5 秒 1 次；账户配置与费率为每用户 2 秒 5 次；账户产品目录为每用户及类别 2 秒 20 次。Global 与 US 区域的 demo REST 域名分别为 `openapi.okx.com` 与 `us.okx.com`，两者仍需 demo 请求头，不能依域名推断环境。业务错误码 `50011` 为限速，`50102` 为时间偏差；HTTP 200 仍须检查顶层 `code`。费率选择以 `groupId` 和 `feeGroup` 为准，旧顶层 maker/taker 字段不能默认为适用费率。`crates/okx-client` 已按上述字段做假服务测试；真实地区账户、模拟盘权限及响应样本留待 DEV-013 显式验收。文档中的目录 GET 返回数组，本阶段未发现该两类接口的游标分页参数；出现分页字段时适配器需先升级测试和解析规则。

### DEV-013 现场只读目录核对（2026-09-25）

再次核对[公共与账户产品目录](https://www.okx.com/docs-v5/en/#trading-account-rest-api-get-instruments)、[账户配置](https://www.okx.com/docs-v5/en/#trading-account-rest-api-get-account-configuration)、[费率组](https://www.okx.com/docs-v5/en/#trading-account-rest-api-get-fee-rates)和[变更日志](https://www.okx.com/docs-v5/log_en/)。在已授权的 Global 模拟账户上运行固定 GET-only 验收：系统时间、账户配置、SPOT/SWAP 账户目录、两个白名单公共规格及其费率组均通过有类型解析、账户绑定和时效校验。实际 `acctLv=2`、`posMode=long_short_mode`；无原始账户响应或密钥进入仓库。另一地区和真实净仓账户尚未现场验证，详见[阶段 1 验收记录](stage1-acceptance.md)。
