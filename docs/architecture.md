...
上图是**目标目录**，不是当前磁盘快照。当前已存在 `.gitignore`、`config/README.md`、`config/env.example`、`deploy/compose/README.md` 和 `docs/*.md`；其余条目均待实现时创建，不能据此运行 `cargo` 或 Compose。每个未逐项展开的 `crates/*` 也是标准库 package，至少有 `Cargo.toml` 与 `src/lib.rs`；每个 `apps/*/runner` 和单 package 应用有 `src/main.rs`。所有 package 在创建时增加自己的 `README.md`，每个实际实现的 Rust module 维护 `//!` 文档，规则见 [模块文档规范](module-documentation.md)。`src/ports.rs` 是所属 `usecases` 库的 module，不是独立 package。实际创建 workspace 时，根 `Cargo.toml` 的 `[workspace].members` 显式列出所有嵌套 package，根目录不设 `[package]` 或 `src/main.rs`。
